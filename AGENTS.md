...
- Operator class names: `KMTOOLS_OT_*`
- Operator `bl_idname`: `keymap_tools.*`
- Panel class names: `KMTOOLS_PT_*`
- Panel `bl_idname`: `KMTOOLS_PT_*`
- PropertyGroup class names: `KMTOOLS_PG_*`
- Menu/Header/UIList classes must also use `KMTOOLS_*` prefixes.
...
## Blender Assets
- geometry nodes都从 @blendfiles\ 内的.blend 文件引用/导入。避免用python写死节点，保持兼容性。 
- geometry nodes使用blender mcp工具修改。
...
- Module-level `register()` / `unregister()` are allowed only for non-class hooks such as attaching `PointerProperty` to `Scene`.
...
- Blender check: run Blender 5.0+ with this add-on enabled and inspect the View3D Sidebar `Concept Tools` tab.
- geometry nodes: 不能有deprecated的节点